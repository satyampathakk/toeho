  ...
  const fetchMyTeachers = async () => {
    try {
      const response = await fetch(
        `${BACKEND_URL}/students/${user?.username}/teachers`
      );
      if (response.ok) {
...
const styles = StyleSheet.create({
  ...
  emptyText: { fontSize: 16, color: 'rgba(255,255,255,0.7)', marginBottom: 24 },
  browseButton: { borderRadius: 12, overflow: 'hidden' },
  browseButtonGradient: { paddingVertical: 14, paddingHorizontal: 24 },
